...
export default {
	...
	computed: {
		...mapGetters("Code", ["getBrandsData", "getBrandModelsData", "getPaintedStatusData",
			"getColorsData", "getGearTypesData", "getOilTypesData", "getCurrencyData", "getRentDriverStatusData"]),
		...mapGetters("Vehicles", ["getCreateUpdateVehicleData"]),
	},
	methods: {
		...mapActions("Code", ["GetBrands", "GetBrandModels", "GetPaintedStatus",
			"GetColor", "GetGearType", "GetOilType", "GetCurrency", "GetRentDriverStatus"]),

		...mapActions("Vehicles", ["UpdateRentVehicle"]),
		...
		async mapData() {
			...
			const ds = this.data.driverStatus ?? this.data.DriverStatus;
			this.data.driverStatus = ds != null && !Number.isNaN(Number(ds)) ? Number(ds) : 0;
			const pt = this.data.paintedTypeIdCfk ?? this.data.PaintedTypeIdCfk ?? this.data.paintedStatusIdCfk;
			this.data.paintedTypeIdCfk = pt != null && !Number.isNaN(Number(pt)) ? Number(pt) : 0;
		...
		updateFunc() {
			...
			const payload = this.buildRentVehiclePayload();

			// Keep loading active until the request finishes (success or error)
			this.UpdateRentVehicle(payload)
				.then((Response) => {
					...
					$('#confirm').modal('hide');
					this.$router.push({ name: "profile_my_rent_vehicles" });
				})
				.catch((error) => {
		...
		buildRentVehiclePayload() {
			const isHiddenPrice = !!this.data.isHiddenPrice;
			const safePrice = isHiddenPrice ? 0 : Number(this.data.price || 0);
			const safeCurrency = isHiddenPrice ? 0 : Number(this.data.currency || 0);

			return {
				id: Number(this.data.id || 0),
				brandIdFk: Number(this.data.brandIdFk || 0),
				modelIdFk: Number(this.data.modelIdFk || 0),
				year: Number(this.data.year || 0),
				colorIdCFK: Number(this.data.colorIdCFK || 0),
				paintedTypeIdCfk: Number(this.data.paintedTypeIdCfk || this.data.paintedStatusIdCfk || 0),
				paintedStatusIdCfk: Number(this.data.paintedStatusIdCfk || 0),
				price: safePrice,
				currency: safeCurrency,
				gearTypeIdCfk: Number(this.data.gearTypeIdCfk || 0),
				oilTypeIdCfk: Number(this.data.oilTypeIdCfk || 0),
				image: this.data.image || "",
				images: Array.isArray(this.data.images) ? this.data.images : [],
				moreInfo: this.data.moreInfo || "",
				bestThreeCategories: Array.isArray(this.data.bestThreeCategories) ? this.data.bestThreeCategories : [],
				extinsionCategory: Array.isArray(this.data.extinsionCategory) ? this.data.extinsionCategory : [],
				driverStatus: Number(this.data.driverStatus || 0),
				isHiddenPrice,
			};
		},
		checkUpdateValidation() {
			if (this.data.id == 0) {
				this.$moshaToast('خطأ في تحديد المركبة', {
					hideProgressBar: 'false',
					position: 'top-center',
					showIcon: 'true',
					swipeClose: 'true',
				...
				return false;
			} else if (!this.data.driverStatus || this.data.driverStatus === 0) {
				this.$moshaToast('إختر حالة السائق / التوصيل', {
					hideProgressBar: 'false',
					position: 'top-center',
					showIcon: 'true',
					swipeClose: 'true',
			...
			// 	return false;
			// } 
			else if (this.data.images.length > 0 && this.data.images.length < 6) {
	...
	<div class="col-12 col-lg-9 add_vehicle">
		<div class="container white_card px-4 pt-4 pb-0 mt-3 mt-lg-0 right-side">
			...
			<div class="page-header">
				<div class="row">
					<div class="col-sm-12">
						<h3 class="page-title">تعديل مركبة الأجار
						</h3>
					</div>
				</div>
			</div>
			...
			<div class="row ">
				<div class="row form-row justify-content-center">
					...
					<div class="col-12 col-md-6">
						<div class="form-group">
							<label>حالة السائق / التوصيل</label>
							<select v-model.number="data.driverStatus"
								class="form-control mt-2 mb-4  py-3 text-start list_link gray-inp">
								<option :value="0">-- إختر حالة السائق --</option>
								<option v-for="item in getRentDriverStatusData" :key="item.id" :value="item.id">
									{{ item.name }}
								</option>
							</select>
						</div>
					...
					<div class="col-12">
						<BorderBox title="سعر الإجار والعملة" variant="rent">
							<div class="form-group">
								<label class="price-privacy-toggle" for="rentPriceHiddenEdit">
									<input id="rentPriceHiddenEdit" v-model="data.isHiddenPrice"
								...
								<p v-if="data.isHiddenPrice" class="price-privacy-hint">
									لن يُعرض سعر الإجار أو العملة للزوار. لا يُشترط إدخال السعر أو اختيار العملة للإرسال.
								</p>
							</div>
					...
					<div class="col-12" v-on:click="toggleCropperModal()">
						<div class="box py-2 px-3 mb-3 rounded-3 cropper">
							<label class="w-100 h-100">
								<div class="icon"></div>
								<h5 class="page-title"> الصورة الرئيسية للمركبة</h5>
								<img loading="lazy" src="/images/green-add-button.png" alt=""
					...
					<div class="col-12">
						<div class="box py-2 px-3 mb-3 rounded-3 drop-images">
							<FilePond ref="pond" imagePreviewMaxHeight="50" imagePreviewMinHeight="20" min-files="6"
								label-idle="...أسقط الصور هنا " allow-multiple="true"
			...
			<div class="mb-5">
				<button class="add full" data-bs-toggle="modal" data-bs-target="#confirm">حفظ التعديلات</button>
			</div>
		</div>
	</div>
	...
	<div class="modal fade" id="confirm" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<p>هل أنت متأكد من التعديلات وصحة محتواها!</p>
				</div>
				<div class="modal-footer">
					<button type="button" v-on:click="updateFunc()" class="btn btn-primary">
						تأكيد
					</button>
				</div>
			</div>
