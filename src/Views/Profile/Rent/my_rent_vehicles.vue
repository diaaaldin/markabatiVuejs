...
export default {
  ...
  methods: {
    ...mapActions("Vehicles", ["GetMyRentVehicles", "GetRentVehicleForUpdate", "UpdateRentVehicleImage360", "DeleteRentVehicle", "UpdateRentVehicleStatus"]),
    ...
    initFunc() {
      ...
      this.GetMyRentVehicles(this.data.statusId).then(Response => {
        loading.close();
      }).catch(error => {
    ...
    GetData() {
      this.GetMyRentVehicles().then(Response => {
      }).catch(error => {
        this.$moshaToast(error?.response?.data?.message || 'An error occurred', {
          hideProgressBar: 'false',
          position: 'top-center',
    ...
    deleteFunc() {
      if (this.checkDeleteValidation()) {
        ...
        this.DeleteRentVehicle(this.data.id).then(Response => {
    ...
    Changeimg360Func() {
      if (this.checkChangeimg360Validation()) {
        ...
        this.UpdateRentVehicleImage360(this.dataimg360).then(Response => {
    ...
    selectItemForUpdate(id) {
      ...
      this.GetRentVehicleForUpdate(id).then(Response => {
        this.$router.push({ name: "profile_update_rent_vehicle" });
        loading.close();
      }).catch(error => {
        this.$moshaToast(error?.response?.data?.message || 'An error occurred', {
          hideProgressBar: 'false',
    ...
    asSoldFunc() {
      if (this.checkasSoldValidation()) {
        ...
        this.UpdateRentVehicleStatus(data).then(Response => {
    ...
    OpenFullScreenFunc(id) {
      ...
      if (foundVehicle) {
        this.selectedVehicle.images = Array.isArray(foundVehicle.images) ? foundVehicle.images : [];
      }
...
<template>
  ...
  <div class="col-12 col-lg-9 myvehicles">
    <div class="ads">
      <div class="add">
        <router-link :to="{ name: 'profile_add_rent_vehicle' }" class="option">{{
          $t('btn_add_rent_vehicle') }}
    ...
    <div class="container white_card px-4 pt-4 pb-0 mt-3 mb-3 mt-lg-0 right-side">
      <div v-if="!getMyVehiclesData || getMyVehiclesData.length === 0"
        class="alert alert-danger mt-3 mb-3 d-flex justify-content-center">
